...
function Navbar({
  ...
  const teamId = suppliedTeamId || params.teamId
  ...
  const isPlayer = currentUser?.account_type === 'player'

  const isManager = currentUser?.account_type === 'manager'
   ...
   */
  const isApprovedPlayer = isPlayer && Boolean(teamId)
  ...
  async function handlePayments() {
    if (!teamId || openingStripe) return
    ...
    try {
      ...
      const endpoint = statusData.setup_complete
        ? 'stripe_dashboard'
        : 'stripe_connect'
      ...
      if (!stripeResponse.ok) {
        setNavbarError(
          stripeData.error || 'Unable to open payments.',
        )
        return
      }
          ...
          <div className="navbar-account">
            ...
            >
              ...
              <span className="navbar-user-details">
                <strong>
                  {currentUser?.first_name || 'My account'}
                </strong>
