...
function ReportModal({
  ...
  async function handleSubmit(event) {
    ...
    if (!reason) {
      setErrorMessage('Please choose a reason for the report.')
      return
    }
    ...
    if (!token) {
      setErrorMessage('Please sign in again before sending a report.')
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          getErrorMessage(data, 'Unable to send this report.'),
        )
      }
      ...
      setErrorMessage(
        error.message || 'Unable to send this report.',
      )
    } finally {
    ...
    >
      ...
      >
        <header className="safety-modal-heading">
          <div>
            <p className="safety-modal-label">Safety report</p>
            <h2 id="report-modal-title">Report {targetLabel}</h2>
            <p>
              Your report is private and will be reviewed by the
              MatchMuster moderation team.
            </p>
          </div>
        ...
        <form className="safety-modal-form" onSubmit={handleSubmit}>
          ...
          <label htmlFor="report-details">
            ...
            <small>{details.length}/1000 characters</small>
          </label>
