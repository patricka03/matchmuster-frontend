...
function SendAvailabilityPage() {
  ...
  useEffect(() => {
    async function loadReminderPage() {
      ...
      try {
        ...
        if (!matchResponse.ok) {
          throw new Error(
            matchData.error || 'Unable to load this fixture.'
          )
        }
        ...
        setErrorMessage(
          error.message || 'Unable to load this fixture.'
        )
      } finally {
        setLoading(false)
      }
  ...
  async function handleSendReminder() {
    ...
    try {
      ...
      if (!response.ok) {
        let message = 'Unable to send the availability reminder.'
      ...
      navigate(
        `/teams/${teamId}/matches/${matchId}/availabilities`,
        {
          ...
          state: {
            successMessage:
              data.message || 'Availability reminder sent.',
          },
        }
      )
    } catch (error) {
      setErrorMessage(
        error.message || 'Unable to send the availability reminder.'
      )
    } finally {
      setSending(false)
    }
        ...
        <section className="send-availability-container">
          ...
          {match && (
            <div className="send-availability-card">
              ...
              <h1>Send availability reminder?</h1>

              <p className="send-availability-description">
                This will remind approved players who have not yet
                submitted their availability for this fixture.
              </p>
              ...
              <p className="send-availability-notice">
                Only players who have not responded will receive this
                reminder.
              </p>
